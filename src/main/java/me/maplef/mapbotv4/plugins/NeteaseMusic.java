...
public class NeteaseMusic implements MapbotPlugin {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(Main.getInstance(), () -> {
            ...
            while (true) {
                ...
                if (responseJson.getInteger("code") == 803) {
                    Objects.requireNonNull(bot.getGroup(groupId))
                            .sendMessage(new PlainText(responseJson.getString("message")));
                    try {
    ...
    private MessageChain logout() {
        ...
        if (logoutResponse.getInteger("code") == 200) {
            ...
            return MessageUtils.newChain(new PlainText("退出登录失败：" + logoutResponse.toJSONString()));
    }
    ...
    private JSONObject loginStatus() {
        ...
        JSONObject responseData = NeteaseMusicUtils.get("/login/status", null, NeteaseMusicUtils.getCookie())
                .getJSONObject("data");
    ...
    private MessageChain search(String songName) {
        ...
        for (int i = 0; i <= 4; i++) {
            ...
            for (Object author : authors)
                authorListBuilder.append(((JSONObject) author).getString("name")).append(", ");
    ...
    private MessageChain sendMusic(int id) {
        NeteaseMusicUtils.SendMode sendMode = NeteaseMusicUtils.SendMode
                .valueOf(config.getString("netease-cloud-music.music.send-mode"));
        ...
        if (sendMode == NeteaseMusicUtils.SendMode.both) {
            Objects.requireNonNull(bot.getGroup(groupId))
                    .sendMessage(sendCard(id, musicUrl, musicName, musicAuthorList, musicImageUrl));
            return sendVoice(musicUrl, groupId);
        } else
            return sendCard(id, musicUrl, musicName, musicAuthorList, musicImageUrl);
    }

    private MessageChain sendCard(int id, String musicUrl, String musicName, ArrayList<String> musicAuthors,
    ...
    private MessageChain sendVoice(String musicUrl, long groupId) {
        ...
        if (!NeteaseMusicUtils
                .mp3ToAmr(new File(Main.getInstance().getDataFolder() + File.separator + "tempMusic.mp3"))) {
            return MessageUtils.newChain(new PlainText("转换失败"));
        }

        try {
            ExternalResource externalResource = ExternalResource.create(
                    new File(Main.getInstance().getDataFolder().getAbsoluteFile() + File.separator + "tempMusic.amr"));
    ...
    @Override
    public MessageChain onEnable(@NotNull Long groupID, @NotNull Long senderID, Message[] args,
            @Nullable QuoteReply quoteReply) throws Exception {
    ...
    public Map<String, Object> register() throws NoSuchMethodException {
        ...
        commands.put("netease",
                NeteaseMusic.class.getMethod("onEnable", Long.class, Long.class, Message[].class, QuoteReply.class));
